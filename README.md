...
This is the main simulation program, which can be run directly to perform computer simulation, generate the photoelectric signal intensity matrix IntentityMat.m, and store it in the folder IntentityMat_v3
...
## restruction.m
This is an image reconstruction program that reads Intensity Mat. m, outputs the reconstructed image and corresponding spectrogram, as well as SSIM and PSNR at different sampling rates
## GetMyPattern_v3_5_posneg. m and myDither_v5. m
Exponential Multi Gray Scale Calculation Weighted Jitter Algorithm
...
## simulation_v3_posneg.m   
这是仿真主程序，直接运行即可进行计算机仿真, 生成光电信号强度矩阵 IntensityMat.m 并存放在文件夹 IntensityMat_v3
...
## restruction.m
这是图像重建程序，读取 IntensityMat.m ，输出重建的图像和对应的频谱图，以及不同采样率下的SSIM和PSNR
