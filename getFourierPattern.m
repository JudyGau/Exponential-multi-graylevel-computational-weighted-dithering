...
%   Detailed explanation goes here
    ...
    switch PATTERN_GENERATION_MODE
        ...
        case 2
            ...
            Pattern = (amp*exp(1i*(2*pi*(fx*X+fy*Y)+initPhase))+1)/2;      % 将坐标代入到复指数函数中
...
% [X,Y]=meshgrid(linspace(0,nCol-1,nCol), linspace(0,mRow-1,mRow));%循环产生等间距的x,y坐标
% Pattern = exp(-1i*2*pi*(fx*X+fy*Y));      % 将坐标代入二维到复指数函数中
% cosPattern = real(Pattern);               %得到傅里叶基底图案的实部
% sinPattern = imag(Pattern);              %得到傅里叶基底图案的虚部
