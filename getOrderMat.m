...
function OrderMat = getOrderMat(mRow, nCol, PathStr,SpectralCoverage)
if mRow > nCol
    nPixel  = mRow;
else
    nPixel = nCol;
...
case 'fourArc'
    PathMat = FourArcPath(nPixel, nPixel, floor(nPixel/2)+1, floor(nPixel/2)+1, ceil(nPixel*2/(2-sqrt(2))*SpectralCoverage));
end
